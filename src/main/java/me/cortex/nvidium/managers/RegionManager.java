...
public class RegionManager {
		...
		while (!this.dirtyRegions.isEmpty()) {
			...
			if (region.isRemoved) {
				if (this.regions[region.id] == null) {
					// No new region replaced this id yet; clear its GPU metadata to prevent rendering stale data
	...
	// Returns a pointer to the section's data buffer.
	// Valid only until the next call on this instance; marks the region dirty.
	...
	public void removeSection(int sectionId) {
		...
		int sectionPos = sectionId; // save position index before overwriting
		sectionId = region.pos2id[sectionId];
		...
		if (endId != sectionId) {
			...
			// Update the section ref-id encoded in the metadata word
	...
	private static class Region {
		...
		private Region(int id, int rx, int ry, int rz) {
			...
			// Initialize translucency index fields to -1
			for (int i = 0; i < 256; i++) {
