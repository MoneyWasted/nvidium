...
public class SectionManager {
	...
	public void uploadIndexBuffer(IntBuffer indexBuffer, int[] quadCountData, long upload) {
		...
		for (var facing : ModelQuadFacing.values()) {
			for (int i = 0; i < quadCountData[facing.ordinal()]; i++) {
				// 1 index per quad (6 indices per quad / 4 vertices per quad)
	...
	public void uploadChunkSort(ChunkSortOutput sortOutput) {
		...
		if (quadCountData == null) { // section not loaded yet
			return;
		}

		// Integrity check: guard against race condition where translucencyQuadCounts was overwritten by a rebuild
		...
		{
			// Index data is stored in the terrain buffer: divide by 6 to get one index per quad, then scale by vertex format size
	...
	public void uploadChunkBuildResult(ChunkBuildOutput result) {
		...
		// bits 18-25: section ref id (used for translucency sorting/rendering); bits 26-31: free
