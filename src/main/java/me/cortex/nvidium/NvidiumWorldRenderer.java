...
public class NvidiumWorldRenderer {
	...
	// Max memory the GPU can use to store geometry, in MB
	private long maxGeometryMemory;
	private long lastMemorySampleTime;

	public NvidiumWorldRenderer() {
		// 32 MB upload buffer, 8 MB download buffer over 3 frames
		this.uploadStream = new UploadingBufferStream(device, 32_000_000);
		this.downloadStream = new DownloadTaskStream(device, 3, 8_000_000);

		updateAllowedMemory();
		int vertexSize = Nvidium.config.use_sodium_vertex_format
			? ChunkMeshFormats.COMPACT.getVertexFormat().getVertexSize()
			: NvidiumCompactChunkVertex.STRIDE;
		this.sectionManager = new SectionManager(device, maxGeometryMemory * 1024 * 1024, uploadStream, vertexSize, this);
	...
	public void renderFrame(TerrainRenderPass pass, Viewport viewport, FogParameters fogParameters, ChunkRenderMatrices matrices, double x, double y, double z, GpuSampler terrainSampler) {
		...
		while (sectionManager.terrainAreana.getUsedMB() > (maxGeometryMemory - 100)) {
	...
	public void addDebugInfo(ArrayList<String> debugInfo) {
		...
		boolean sparse = Nvidium.SUPPORTS_PERSISTENT_SPARSE_ADDRESSABLE_BUFFER;
		long memUsed = sparse ? sectionManager.terrainAreana.getAllocatedMB() : sectionManager.terrainAreana.getUsedMB();
		debugInfo.add("Mem" + (sparse ? "" : " (fallback)") + ": " + memUsed + "/" + this.maxGeometryMemory
	...
	private void updateAllowedMemory() {
		if (Nvidium.config.automatic_memory) {
			long available = glGetInteger(GL_GPU_MEMORY_INFO_CURRENT_AVAILABLE_VIDMEM_NVX) / 1024;
			long currentUsage = sectionManager == null ? 0 : sectionManager.terrainAreana.getMemoryUsed() / (1024 * 1024);
			maxGeometryMemory = Math.max(2048, available + currentUsage - 1024); // Reserve 1 GB, minimum 2 GB
		} else {
			maxGeometryMemory = Nvidium.config.max_geometry_memory;
		}
	}
